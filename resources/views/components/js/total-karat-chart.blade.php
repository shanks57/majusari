<script>
    ...
    const htmlLegendPlugin = {
        ...
        afterUpdate(chart, args, options) {
            ...
            items.forEach((item, i) => {
                ...
                // Add item count (pcs) if rawData is available
                if (chart.rawData && chart.rawData[i]) {
                    const pcsValue = document.createElement('span');
                    pcsValue.style.marginLeft = '10px';
                    pcsValue.textContent = `(${chart.rawData[i].total_items} pcs)`;
                    pcsValue.style.color = '#666';
                    pcsValue.style.fontSize = '0.9em';
                    spanValue.appendChild(pcsValue);
                }
    ...
    fetch('/get-weight-by-gold-rate')
        ...
        .then(data => {
            ...
            // Create chart object and store raw data before initialization
            ...
            // Store raw data immediately after chart creation
            goldRateChart.rawData = data;
            
            // Force plugin update to ensure rawData is available
            setTimeout(() => {
                htmlLegendPlugin.afterUpdate(goldRateChart, {}, {containerID: 'legend-container-tiga'});
            }, 0);
        })
        .catch(error => console.error('Error fetching data:', error));
