            ...
            <form :action="`{{ route('master.employees.update', $employee->id) }}`" method="post">
                ...
                <div class="px-4 overflow-y-auto">
                    ...
                    <div class="w-full mb-4">
                        ...
                        <textarea id="address" name="address" x-model="form.address" class="block w-full px-4 py-3 text-base mt-1.5 border-gray-200 rounded-lg focus:border-[#79799B] disabled:opacity-50 disabled:pointer-events-none text-black placeholder:text-[#667085]" rows="3" placeholder="Masukkan Alamat" required></textarea>
                        @error('address')
                        <span class="mt-1 text-sm text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
