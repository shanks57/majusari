<div x-data="{ form: { name: '', username: '', phone: '', debt_receipt: '', wages: '', address: '', status: false } }"
        ...
        <div class="flex flex-col p-6 bg-white shadow-lg pointer-events-auto gap-y-4 rounded-xl">
            ...
            <form action="{{ route('master.employees.store') }}" method="post">
                ...
                <div class="flex items-center justify-end px-4 gap-x-2">
                    <button type="submit"
                        :disabled="!form.name || !form.username || !form.phone || !form.debt_receipt || !form.wages || !form.address"
                        class="flex items-center justify-center px-4 py-3 text-sm font-medium leading-5 rounded-lg bg-[#7F56D9] text-white"
                        :class="{ 'opacity-50 cursor-not-allowed': !form.name || !form.username || !form.phone || !form.debt_receipt || !form.wages || !form.address }">
                        <span>Simpan</span>
