<div x-data="{ open: @js(session('success-edit') ? true : false) }" x-show="open" x-on:click.away="open = false"
    ...
    <div class="w-full max-w-md p-6 mx-4 bg-white border border-[#D9D9D9] shadow-lg rounded-xl">
        <div class="flex flex-col items-center justify-between">
            <div class="pb-4 mb-4 border-b border-[#D0D5DD] w-full text-center">
                <span class="text-xl font-medium text-[#344054] text-center">Ubah Pegawai</span>
            </div>
            <div class="mb-4">
                <p class="text-xl font-medium text-black font-inter">{{ session('success-edit') }}</p>
            </div>
            ...
            <button @click="open = false" type="button"
                    class="flex items-center mt-4 px-4 py-3 gap-1.5 text-sm font-medium rounded-lg bg-[#6634BB] text-[#F8F8F8]">
                    <span>Tutup</span> <i class="ph-bold ph-x text-[#F6F6F6]"></i>
            </button>
        </div>
    </div>
</div>
