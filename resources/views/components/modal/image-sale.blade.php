<div id="hs-image-sale-modal-{{ $sale->id }}"
    class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none"
    role="dialog" tabindex="-1" aria-labelledby="hs-image-sale-modal-{{ $sale->id }}-label">
    <div
        ...
        <div
            ...
            <div class="flex items-center justify-center pb-4 border-b">
                <h3 id="hs-image-sale-modal-{{ $sale->id }}-label" class="text-xl font-semibold text-[#344054]">
                    Tampilan Penuh Etalase
                </h3>
            </div>
            <div class="flex justify-center">
                <img src="storage/{{ $sale->goods->image }}" class="rounded size-80"
                    alt="{{ $sale->goods->name }}">
            </div>
            <div class="flex justify-center text-sm text-[#151617] gap-3">
                <p class="">{{ $sale->goods->name }} - {{$sale->goods->color}}</p>
                <p>{{ $sale->goods->merk->company }}</p>
            </div>
