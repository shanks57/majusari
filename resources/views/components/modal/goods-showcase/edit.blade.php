<div x-data="{ 
        form: {
            ...
            size: '{{ $goodShowcase->size }}', 
            dimensions: '{{ $goodShowcase->dimensions }}', 
            merk_id: '{{ $goodShowcase->merk_id }}', 
            ask_rate: '{{ $goodShowcase->ask_rate }}', 
            bid_rate: '{{ $goodShowcase->bid_rate }}', 
            ask_price: '{{ $goodShowcase->ask_price }}', 
    ...
    role="dialog" tabindex="-1" aria-labelledby="edit-modal-label" id="hs-edit-modal-{{ $goodShowcase->id }}">
    <div
        ...
        <div class="flex flex-col p-6 bg-white shadow-lg pointer-events-auto gap-y-4 rounded-xl">
            ...
            <form action="{{ route('goods.showcaseUpdate', $goodShowcase->id) }}" method="post"
                enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="px-4">
                    <div x-data="imageUploader('{{ $goodShowcase->image ? asset('storage/' . $goodShowcase->image) : '' }}')"
