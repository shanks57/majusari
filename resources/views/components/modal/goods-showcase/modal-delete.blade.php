<div id="hs-delete-modal-{{ $goodShowcase->id }}"
    class="hs-overlay hidden fixed top-0 end-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none mr-8"
    role="dialog" tabindex="-1" aria-labelledby="hs-delete-modal-{{ $goodShowcase->id }}-label">
    <div
        ...
        <div class="flex flex-col gap-4 p-4 bg-[#FCFCFD] border shadow-sm pointer-events-auto rounded-xl">
            <div class="flex items-center justify-center border-[#EAECF0]">
                <h3 id="hs-delete-modal-{{ $goodShowcase->id }}-label" class="text-lg text-[#344054] leading-7">
                    Yakin mau hapus data?
                </h3>
            </div>
            <div class="flex items-center justify-center gap-4">
                <form action="{{ route('goods-showcase.destroy', ['id' => $goodShowcase->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
