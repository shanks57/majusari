    ...
    <div
        ...
        <div class="flex flex-col gap-4 p-4 bg-[#FCFCFD] border shadow-sm pointer-events-auto rounded-xl">
            <div class="flex items-center justify-center border-[#EAECF0]">
                <h3 id="hs-delete-modal-{{ $etalase->id }}-label" class="text-lg text-[#344054] leading-7">
                    Yakin mau hapus data?
                </h3>
            </div>
            <div class="flex items-center justify-center gap-4">
                <form action="{{ route('master.showcase.destroy', ['id' => $etalase->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
